...
| File | Change |
|------|--------|
| `src/generation/life_story_generator.py` | Added import; wrapped `messages.create` |
| `src/generation/narrative_generator.py` | Added import; wrapped `messages.create` |
| `src/cognition/decide.py` | Added import; wrapped both `messages.create` calls |
| `src/cognition/perceive.py` | Added import; wrapped both `messages.create` calls |
| `src/cognition/reflect.py` | Added import; wrapped `messages.create` |

---

## 2. Lines Changed (before → after)

### `src/generation/life_story_generator.py` (~line 272)
...
response = await self.llm.messages.create(
    ...
    messages=[{"role": "user", "content": user_prompt + extra_note}],
)
```
After:
```python
...
### `src/generation/narrative_generator.py` (~line 145)
...
### `src/cognition/decide.py` (~lines 271 and 283)
...
```
Attempt 2 changed identically (same kwargs).

### `src/cognition/perceive.py` (~lines 184 and 197)
...
```
Attempt 2 changed identically.

### `src/cognition/reflect.py` (~line 227)
...
```
$ grep -l "api_call_with_retry" src/generation/life_story_generator.py src/generation/narrative_generator.py src/cognition/decide.py src/cognition/perceive.py src/cognition/reflect.py
src/generation/life_story_generator.py
src/generation/narrative_generator.py
src/cognition/decide.py
src/cognition/perceive.py
src/cognition/reflect.py
```
...
```
249 passed, 15 skipped in 1.45s
```

No failures. No regressions. All 249 previously-passing tests continue to pass.
