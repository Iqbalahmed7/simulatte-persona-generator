...
Sprint 10 builds Sarvam. Your job: write the gate tests that verify the Sarvam activation pre-check conditions, anti-stereotypicality rules, and the structural integrity of the enrichment record schema.

---

## File: `tests/test_sarvam_gates.py`
...
- Test 3 uses a `pytest.raises(Exception)` + `raise ValueError` pattern — it should pass.
- Test 7 tests `extra="forbid"` on Pydantic — raises `ValidationError`.
