# SPRINT 10 BRIEF — CODEX
**Role:** Sarvam Enricher (LLM calls)
**Sprint:** 10 — Sarvam Indian Cultural Realism Layer
**Spec ref:** Master Spec §15, SIMULATTE_SARVAM_TEST_PROTOCOL.md CR1–CR4
...
Sprint 10 builds the Sarvam Indian Cultural Realism Layer. Your job: write the `SarvamEnricher` class that takes a finalized `PersonaRecord` and returns a `SarvamEnrichmentRecord` with culturally enriched narratives.

**Key architectural rule:** Sarvam NEVER modifies the PersonaRecord. It reads it as a read-only input and produces a separate enrichment record. The PersonaRecord must be identical before and after enrichment.

Types are defined in `src/sarvam/types.py` and `src/sarvam/config.py` (Cursor).

---

## File: `src/sarvam/enrichment.py`
...
### Prompt design (implement these two methods):
...
### Complete `enrich()` implementation:

```python
async def enrich(self, persona, config):
...
## File: `tests/test_sarvam_enrichment.py`

### Test 1: Enricher returns SarvamEnrichmentRecord (integration)
...
async def test_enricher_returns_record():
    ...
    # Override location to India
    from src.schema.persona import Location
...
### Test 2: Prompt builder produces non-empty prompt
...
### Test 3: Parse response handles markdown code block
...
### Test 4: Parse response handles invalid JSON gracefully
...
- LLM test (Test 1) is `@pytest.mark.integration` — skip without `--integration`.
- Structural tests (2, 3, 4) pass without `--integration`.
- `enrich()` must NEVER mutate the persona argument.
- Fallback behaviour on parse failure: return record with original narratives unchanged (not raise).
- Model: use `config.model` (not hardcoded).
...
4. Test results (structural 3/3 pass; integration 1 skipped or pass)
5. Full suite result
6. Known gaps
