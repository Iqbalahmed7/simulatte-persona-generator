# SPRINT 7 BRIEF — GOOSE
**Role:** Simulation End-to-End Test (BV3, BV6)
**Sprint:** 7 — Temporal Simulation Modality
**Spec check:** Validity Protocol BV3 (temporal consistency), BV6 (believable consistency vs rigidity)
...
One file: `tests/test_simulation_bv.py`. Integration tests for BV3 and BV6 against the temporal simulation runner. These make real LLM calls.

---

## File: `tests/test_simulation_bv.py`
...
- Both tests are `@pytest.mark.integration` — real LLM calls.
- Skip automatically without `--integration`.
- Keep stimuli short to minimise API cost.
- BV3 tolerance: ±10 confidence points on the arc check (LLM confidence scores have natural variance).
- BV6 consistent check: `>= 3` OR `len(standard_logs) < 3` guard handles cases where Priya doesn't trigger decide on all 5 turns.
...
2. BV3 — arc design and tolerance rationale
3. BV6 — override scenario design and consistency check
