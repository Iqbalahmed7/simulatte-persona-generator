...
Usage:
    ...
    python3 pilots/littlejoys/regenerate_pipeline.py [--skip-sarvam] [--dry-run] [--count N]
                                                     [--tier deep|signal|volume] [--simulate]
...
Flags:
    ...
    --tier          Model routing tier: deep (default), signal, or volume
    --simulate      Run Stage 6: stimulus simulation pass (makes LLM calls — costs tokens)
"""
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--tier",
        default="deep",
        choices=["deep", "signal", "volume"],
        help="Simulation tier for model routing (default: deep)",
    )
    parser.add_argument(
        "--simulate",
        action="store_true",
        help="Run Stage 6: stimulus simulation pass using the configured tier (makes LLM calls)",
    )
...
def _build_cohort_envelope(personas: list, tier: str = "deep") -> object:
    ...
    calibration_state = CalibrationState(
        ...
        notes=(
            f"Regenerated by regenerate_pipeline.py; tier={tier}; "
            "Sprint 19 features active: noise_injection, core_memory_cache, tiered_simulation"
        ),
    )
...
def _run_validation_and_save(personas: list, dry_run: bool, tier: str = "deep") -> None:
    ...
    envelope = _build_cohort_envelope(personas, tier=tier)
...
# ---------------------------------------------------------------------------
# Stage 6: Simulation pass (optional — gated by --simulate)
# ---------------------------------------------------------------------------

# Three highest-signal LJ touchpoints (from JOURNEY_INSIGHTS research):
# 1. Pediatrician recommendation — single strongest first-purchase trigger
# 2. Close-friend WOM — as powerful as doctor rec for high-trust cohort
# 3. Subscribe-and-Save pricing — top conversion mechanism for hesitant buyers
_LJ_STIMULI = [
    (
        "Your child's pediatrician mentions that Nutrimix can help fill nutritional "
        "gaps in picky eaters and recommends trying it consistently for 4-6 weeks."
    ),
    (
        "A close friend messages you: 'We've been giving our son Nutrimix for a month — "
        "his appetite improved and he's less fussy at dinner. Totally worth it.'"
    ),
    (
        "You see that Littlejoys Nutrimix is offering a Subscribe & Save bundle: "
        "₹649/month instead of ₹799, with free delivery and skip-any-month flexibility."
    ),
]

_LJ_DECISION_SCENARIO = (
    "Should you buy Littlejoys Nutrimix for your child this month?"
)


async def _run_simulation_pass_async(personas: list, tier_str: str) -> None:
    """Stage 6 async inner: run 3 LJ stimuli + decision scenario through run_loop."""
    from src.experiment.session import SimulationTier
    from src.cognition.loop import run_loop

    api_key = os.environ.get("ANTHROPIC_API_KEY")
    if not api_key:
        print("[Stage 6] ERROR: ANTHROPIC_API_KEY not set — cannot run simulation pass.")
        return

    tier = SimulationTier(tier_str)
    total = len(personas)
    print(
        f"[Stage 6] Simulation pass... {total} personas × {len(_LJ_STIMULI)} stimuli "
        f"+ decision (tier={tier_str})"
    )
    print(f"  Decision scenario: '{_LJ_DECISION_SCENARIO}'")
    ...
    for i, persona in enumerate(personas):
        try:
            p = persona
            # Run each stimulus through the cognitive loop (no decision scenario yet)
            for stimulus in _LJ_STIMULI[:-1]:
                p, _ = await run_loop(stimulus=stimulus, persona=p, tier=tier)

            # Final stimulus with decision scenario attached
            p, result = await run_loop(
                stimulus=_LJ_STIMULI[-1],
                persona=p,
                decision_scenario=_LJ_DECISION_SCENARIO,
                tier=tier,
            )

            if result.decision:
                d = result.decision
                # Normalise decision text to a short key (first 5 words)
                key = " ".join(d.decision.split()[:5]).rstrip(".,")
                decision_counts[key] = decision_counts.get(key, 0) + 1
                confidence_vals.append(d.confidence)
                noise_vals.append(d.noise_applied)

        except Exception as exc:
            errors += 1
            if errors <= 3:
                print(f"\n  [Stage 6] Error for {persona.persona_id}: {exc}")

        if (i + 1) % 10 == 0 or (i + 1) == total:
            print(f"\r[Stage 6] {i + 1}/{total} done", end="", flush=True)

    print()  # newline after progress

    # Report
    decided = len(confidence_vals)
    if decided:
        avg_conf = sum(confidence_vals) / decided
        avg_noise = sum(abs(n) for n in noise_vals) / decided
        print(f"  Personas with decision output: {decided}/{total}")
        print(f"  Avg confidence (post-noise):   {avg_conf:.1f}")
        print(f"  Avg |noise| applied:           {avg_noise:.1f}")
        if decision_counts:
            print("  Decision distribution (top 5):")
            for key, count in sorted(decision_counts.items(), key=lambda x: -x[1])[:5]:
                pct = count / decided * 100
                print(f"    {pct:5.1f}%  '{key}...'")
    if errors:
        print(f"  Errors: {errors}/{total} personas failed simulation")


def _run_simulation_pass(personas: list, tier_str: str) -> None:
    """Stage 6: Synchronous wrapper."""
    asyncio.run(_run_simulation_pass_async(personas, tier_str))
...
def main() -> None:
    ...
    count = args.count
    tier = args.tier
    run_simulate = args.simulate and not dry_run

    if dry_run:
        print("=== Littlejoys Regeneration Pipeline [DRY RUN — Stages 1-2 only] ===")
    else:
        tier_label = f"tier={tier}" + (" + simulation pass" if run_simulate else "")
        print(f"=== Littlejoys Regeneration Pipeline [{tier_label}] ===")
    print()
    ...
    # ------------------------------------------------------------------
    _run_validation_and_save(personas, dry_run=False, tier=tier)

    # ------------------------------------------------------------------
    # Stage 6: Simulation pass (optional — only if --simulate was passed)
    # ------------------------------------------------------------------
    if run_simulate:
        print()
        _run_simulation_pass(personas, tier)
