"""tests/test_simulation_e2e.py — S1/S2 Simulation Quality Gate tests.

Sprint 7 — OpenCode (Simulation Report + S1/S2 Quality Gates)

Spec: Master Spec §12 (Simulation Quality Gates S1–S4)

S1: Zero Error Rate — 5-persona trial run completes without error (10 stimuli, no decisions).
S2: Decision Diversity — No single decision > 90% of cohort on any decision turn.

All tests are @pytest.mark.integration — they make real LLM calls.
They skip automatically without --integration flag (conftest.py handles this).
Run with: pytest tests/test_simulation_e2e.py --integration
"""
